...
const DEFAULT_FAVICON = `data:image/svg+xml,${encodeURIComponent(
  '<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32"><rect width="32" height="32" rx="8" fill="#d0d7de"/><text x="16" y="22" text-anchor="middle" font-family="Arial,sans-serif" font-size="14" fill="#57606a">W</text></svg>'
)}`;
...
async function init() {
  ...
  const [tab, currentWindow] = await Promise.all([getCurrentTab(), chrome.windows.getCurrent()]);
